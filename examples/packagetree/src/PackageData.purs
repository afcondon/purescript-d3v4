...
getRepoName :: URL -> RepoName
getRepoName url = fromMaybe "parse error" repo
  where
    subs = split "/" url
    repo = subs !! 3    -- drops the "http:" "" "github.com" parts and the sub-dirs
...
convert ps groups = { nodes: map getNode ps
                    , links: concatMap getLinks ps }
  where
    getLinks :: PackageData -> Array Link
    getLinks { name: name, dependencies: deps } = map (createLink name) deps
      where
        createLink s t = { source: s, target: t, value: 1.0 }
    getNode :: PackageData -> Node
    getNode { name: name, repo: repo } = { id: name, group: groupID } where
      key   = getRepoName repo
      groupID = fromMaybe 0.0 (lookup key groups)

getGroups :: PackageSet -> Map RepoName Number
getGroups ps = fromFoldable $ zip uniqueRepos indices
  where
    uniqueRepos = nub $ foldr (\p acc -> (getRepoName p.repo):acc ) [] ps
...
dummyData = [
  ...
  { name: "tropical",
    ...
    "repo": "https://github.com/paf31/purescript-tropical.git",
    "version": "v2.0.0"
  },
  { name: "tuples",
    "dependencies": [
